...
module.exports = grammar({
  ...
  rules: {
    ...
    source_file: ($) =>
      seq(
        $.syntax,
        optional(repeat(choice($.import, $.package, $.option, $.enum, $.message, $.service, $.empty_statement)))
      ),
    ...
    import: ($) => seq('import', field('modifier', optional(choice('weak', 'public'))), field('path', $.string), ';'),

    // package = "package" fullIdent ";"
    package: ($) => seq('package', field('package_name', $.full_ident), ';'),

    // option = "option" optionName  "=" constant ";"
    // optionName = ( ident | "(" fullIdent ")" ) { "." ident }
    option: ($) => seq('option', $._option_name, '=', $.constant, ';'),

    _option_name: ($) => seq(choice($.identifier, seq('(', $.full_ident, ')')), repeat(seq('.', $.identifier))),

    // enum = "enum" enumName enumBody
    // enumBody = "{" { option | enumField | emptyStatement } "}"
    // enumField = ident "=" [ "-" ] intLit [ "[" enumValueOption { ","  enumValueOption } "]" ]";"
    // enumValueOption = optionName "=" constant
    enum: ($) => seq('enum', $.enum_name, $.enum_body),

    enum_name: ($) => $.identifier,

    enum_body: ($) => seq('{', repeat(choice($.option, $.enum_field, $.empty_statement)), '}'),

    enum_field: ($) =>
      seq(
        field('enum_member_name', $.identifier),
        '=',
        optional('-'),
        $.int_lit,
        optional(seq('[', $.enum_value_option, repeat(seq(',', $.enum_value_option)), ']')),
        ';'
      ),
    ...
    message_body: ($) =>
      seq(
        '{',
        repeat(choice($.field, $.enum, $.message, $.option, $.oneof, $.map_field, $.reserved, $.empty_statement)),
        '}'
      ),
    ...
    range: ($) => seq($.int_lit, optional(seq('to', choice($.int_lit, 'max')))),

    field_names: ($) => seq($.identifier, repeat(seq(',', $.identifier))),

    // messageType = [ "." ] { ident "." } messageName
    message_or_enum_type: ($) => seq(optional('.'), repeat(seq($.identifier, '.')), $.identifier),
    ...
    rpc: ($) =>
      seq(
        ...
        '(',
        optional('stream'),
        field('request_type', $.message_or_enum_type),
        ')',
        'returns',
        '(',
        optional('stream'),
        field('response_type', $.message_or_enum_type),
        ')',
      ...
      seq(
        ...
        repeat(
          seq(
            $.identifier,
            optional(':'),
            choice($.constant, seq('[', $.constant, repeat(seq(',', $.constant)), ']')),
            optional(choice(',', ';'))
          )
        ),
        '}'
    ...
    identifier: ($) => token(seq(letter, optional(repeat(choice(letter, decimal_digit, '_'))))),

    // fullIdent = ident { "." ident }
    full_ident: ($) => seq($.identifier, optional(repeat(seq('.', $.identifier)))),
    ...
    hex_lit: ($) => token(seq('0', choice('x', 'X'), hex_digit, repeat(hex_digit))),
    ...
    float_lit: ($) => {
      ...
      const exponent = seq(choice('e', 'E'), optional(choice('+', '-')), decimals);
    ...
    escape_sequence: ($) =>
      token.immediate(seq('\\', choice(/[^xuU]/, /\d{2,3}/, /x[0-9a-fA-F]{2,}/, /u[0-9a-fA-F]{4}/, /U[0-9a-fA-F]{8}/))),
